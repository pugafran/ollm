...
# Monkey patch Qwen2MoeAttention to bypass the strict head_dim check and derive the
# head dimension from the loaded weights instead of the config (which is where the
# mismatch that triggers the `shape '[1, 30, 2048]' is invalid for input of size 122880`
# error comes from).
...
def new_init(self, config, layer_idx: int | None = None):
    ...
    # Use a safe default based on hidden_size and heads; the actual head_dim used in
    # forward will be derived from the loaded projection weights to avoid shape issues.
    ...
    # Define projection layers without relying on potentially incorrect config.head_dim.
...
def new_forward(
    ...
    # Derive the real head dimension from the projection weights to match loaded checkpoints.
